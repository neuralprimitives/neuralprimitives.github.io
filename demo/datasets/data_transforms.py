...
# Primitive type mapping (used by datasets to map H5 'types' strings -> int IDs)
# Order is fixed and minimal: plane=0, cylinder=1, sphere=2, cone=3 
...
class UpSamplePlanes(object):
    ...
    def __call__(self, plane):
        ...
        if plane.shape[0] < self.n_planes:
            ...
            plane = np.concatenate([plane, padding], axis=0)  # Fixed: specify axis=0
        else:
